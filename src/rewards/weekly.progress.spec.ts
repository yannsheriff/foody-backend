...
function weekDay(offset: number): Date {
  return new Date(
    WEEK.weekStart.getTime() + offset * 86_400_000 + 12 * 3_600_000,
  );
}
...
describe('countQualifyingDays', () => {
  it('counts distinct qualifying days across the week', () => {
    const days = [weekDay(0), weekDay(1), weekDay(2)].map((date) =>
      mkDay({ date }),
    );
    expect(
      countQualifyingDays(
  ...
  it('does not count days after `now` (clamp), today included', () => {
    const days = [weekDay(0), weekDay(1), weekDay(4)].map((date) =>
      mkDay({ date }),
    );
...
describe('resolveWeekly', () => {
  it('wins when the count reaches the target', () => {
    ...
    expect(
      resolveWeekly(def('w-saisie-5'), 5, days, WEEK.weekStart, WEEK.weekEnd)
        .won,
    ).toBe(true);
  });
  it('loses when it falls short', () => {
    const days = [0, 1, 2, 3].map((o) => mkDay({ date: weekDay(o) }));
    const r = resolveWeekly(
      def('w-saisie-5'),
      5,
      days,
      WEEK.weekStart,
...
describe('nouvelles mécaniques', () => {
  it('parfait : 3 repas sans copieux + sport + zéro grignotage', () => {
    ...
    expect(
      qualifiesWeekly(
        def('w-parfait-1'),
        mkDay({ date: weekDay(0), sport_level: 'intense' }),
      ),
    ).toBe(true);
    expect(
      qualifiesWeekly(
        def('w-parfait-1'),
        mkDay({ date: weekDay(0), sport_level: null }),
      ),
    ).toBe(false);
    expect(
      qualifiesWeekly(
    ...
    expect(
      qualifiesWeekly(
        def('w-parfait-1'),
        mkDay({ date: weekDay(0), sport_level: 'intense', snack: 0.5 }),
      ),
    ).toBe(false);
  });
  ...
  it('combo : sport ET dîner léger le même jour', () => {
    ...
    expect(
      qualifiesWeekly(
        def('w-combo-2'),
        mkDay({ date: weekDay(0), sport_level: null, evening_score: 'leger' }),
      ),
    ).toBe(false);
    expect(
      qualifiesWeekly(
